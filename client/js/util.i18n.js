const LANGUAGES = {
	en: {
		...
		translations: {
			...
			'help.what_is_sprintmate': '🔐 What is SprintMate?',			'help.what_is_sprintmate_desc': 'SprintMate is an end-to-end encrypted (E2EE) chat system. Your device encrypts every message and file using AES-256-GCM. For each peer, a shared key is derived from ECDH and strengthened with your room password via HKDF-SHA256, so only intended recipients can decrypt. Separately, the client also negotiates a server-session key to encrypt traffic to the relay (signed server key + local pinning) — the relay only forwards ciphertext and cannot read your plaintext.',
			...
			'help.decentralized_desc': 'The server never decrypts content and can be self-hosted.',			'help.usage_tips': '💡 Usage Tips',
			...
			'help.tip_security_reminder_desc': 'Use HTTPS/WSS. If you see a server-key mismatch warning, do not proceed unless you intentionally changed servers and you trust the new key.',
			'help.tip_password_strategy': 'Password Strategy',
			'help.tip_password_strategy_desc': 'Use a long passphrase (6666666+ chars). Prefer words + numbers/symbols. The password strengthens the E2EE key derivation.',
		}
	},
	zh: {
		code: 'zh',
		...
		translations: {
			...
			'help.decentralized_desc': '服务器不参与加解密，只转发密文。',			'help.usage_tips': '💡 使用技巧',
