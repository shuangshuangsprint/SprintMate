// UI logic for SprintMate web client
// SprintMate 网页客户端的 UI 逻辑
...
function connectionStatusText(status) {
	switch (status) {
		case 'connecting':
			return t('ui.connecting', 'Connecting...');
		case 'connected':
			return t('status.connected', 'Secured');
		case 'reconnecting':
			return t('system.reconnecting', 'Connection lost, reconnecting...');
		case 'disconnected':
...
function ensureConnectionStatusBadge() {
	const headerFlex = document.querySelector('#main-header-center .main-header-flex') || document.getElementById('main-header-center');
...
function applyConnectionStatusBadge() {
	const badge = ensureConnectionStatusBadge();
	if (!badge) return;
	const text = connectionStatusText(currentConnectionStatus);
	if (!text) {
		badge.hidden = true;
		badge.textContent = '';
		badge.className = 'connection-status-badge';
	...
	badge.className = `connection-status-badge is-${currentConnectionStatus}`;
}

export function setConnectionStatusBadge(status = '') {
	currentConnectionStatus = status || '';
	applyConnectionStatusBadge();
}
...
export function setupMobileUIHandlers() {
	...
	function updateMobileBtnDisplay() {
		if (isMobile()) {
			...
			document.body.classList.remove('desktop-hide-left');
			document.body.classList.remove('desktop-hide-right');
		} else {
			if (mobileMenuBtn) mobileMenuBtn.style.display = 'flex';
			if (mobileInfoBtn) mobileInfoBtn.style.display = 'flex';
	...
	if (mobileMenuBtn && sidebar && sidebarMask) {
		mobileMenuBtn.onclick = function(e) {
			...
			if (isMobile()) {
				...
				document.body.classList.toggle('desktop-hide-left');
			}
	...
	if (mobileInfoBtn && rightbar && rightbarMask) {
		mobileInfoBtn.onclick = function(e) {
			...
			if (isMobile()) {
				...
				document.body.classList.toggle('desktop-hide-right');
			}
		};
...
function updatePublicChannelButtonText() {
	const publicBtn = $id('login-public-btn');
	if (!publicBtn) return;
	const label = t('ui.public_channel', 'Public Channel');
	publicBtn.textContent = label;
	publicBtn.setAttribute('title', t('ui.public_channel_auto_join', 'Random username, room: public, no password, auto enter'));
	publicBtn.setAttribute('aria-label', label);
}

function randomPublicUsername() {
	return `Guest${Math.floor(Math.random() * 9000) + 1000}`;
}

function initPublicChannelQuickJoin() {
	const publicBtn = $id('login-public-btn');
	const loginForm = $id('login-form');
	if (!publicBtn || !loginForm) return;

	updatePublicChannelButtonText();

	if (publicBtn.dataset.bound === '1') {
		return;
	}
	publicBtn.dataset.bound = '1';
	...
	publicBtn.addEventListener('click', () => {
		...
		userInput.value = randomPublicUsername();
...
// 初始化登录页颜色滚轮
// Initialize login accent color wheel
export function initLoginAccentWheel() {
	const wheel = document.getElementById('login-color-wheel');
	const thumb = document.getElementById('login-color-thumb');
	...
	function setHue(nextHue) {
		hue = (nextHue + 360) % 360;
		root.style.setProperty('--ios-blue', `hsl(${hue} 100% 50%)`);
		root.style.setProperty('--ios-blue-pressed', `hsl(${hue} 100% 42%)`);
		updateThumb();
	}
...
window.addEventListener('connectionStatusChange', (event) => {
	const status = event && event.detail && typeof event.detail.status === 'string' ? event.detail.status : '';
	setConnectionStatusBadge(status);
});
